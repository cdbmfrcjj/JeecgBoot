<template>
  <a-modal :title="title" :width="width" :open="visible" :confirmLoading="confirmLoading" @cancel="handleCancel"
    :destroyOnClose="true" :afterClose="reset" cancelText="关闭">
...
function open(recordparams) {
  url.value = window._CONFIG['domianURL'] + "/autoApi/execute/" + (recordparams.permission == "disable" ? "public" : "private") + "/" + recordparams.name
  visible.value = true;
}
function handleCopy() {
  if (!url.value) {
