...
import org.jeecg.modules.mobile.mapper.SysAppPermissionMapper;
...
public class SysAppPermissionServiceImpl extends ServiceImpl<SysAppPermissionMapper, SysAppPermission> implements ISysAppPermissionService {
	...
	public void switchVue3Menu() {
		sysAppPermissionMapper.backupVue2Menu();
		sysAppPermissionMapper.changeVue3Menu();
	}

	@Override
	public List<TreeAppModel> queryListByParentId(String parentId) {
		return sysAppPermissionMapper.queryListByParentId(parentId);
	}
	...
	public void deletePermission(String id) throws JeecgBootException {
		...
		if(oConvertUtils.isNotEmpty(pid)) {
			Long count = this.count(new QueryWrapper<SysAppPermission>().lambda().eq(SysAppPermission::getParentId, pid));
			if(count==1) {
	...
	public void editPermission(SysAppPermission sysPermission) throws JeecgBootException {
		...
		if(p==null) {
			...
			Long count = this.count(new QueryWrapper<SysAppPermission>().lambda().eq(SysAppPermission::getParentId, sysPermission.getId()));
			if(count==0) {
				sysPermission.setLeaf(true);
			}
			//----------------------------------------------------------------------
            ...
            if (flag) {
				...
				Long cc = this.count(new QueryWrapper<SysAppPermission>().lambda().eq(SysAppPermission::getParentId, p.getParentId()));
				if(cc==0) {
	...
	@Override
	public List<SysAppPermission> queryByUser(String userId) {
		List<SysAppPermission> permissionList = this.sysAppPermissionMapper.queryByUser(userId);
		//================= begin 开启租户的时候 如果没有test角色，默认加入test角色================
		if (MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL) {
			if (permissionList == null) {
				permissionList = new ArrayList<>();
			}
			List<SysAppPermission> testRoleList = sysAppPermissionMapper.queryPermissionByTestRoleId();
			//update-begin-author:liusq date:20230427 for: [QQYUN-5168]【vue3】为什么出现两个菜单 菜单根据id去重
			for (SysAppPermission permission: testRoleList) {
	...
	public boolean hasPermission(String username, String url) {
		SysAppPermission sysPermission = new SysAppPermission();
	...
	@Override
	public List<SysAppPermission> queryDepartPermissionList(String departId) {
		return sysAppPermissionMapper.queryDepartPermissionList(departId);
	}

	@Override
	public boolean checkPermDuplication(String id, String url,Boolean alwaysShow) {
		QueryWrapper<SysAppPermission> qw=new QueryWrapper();
		qw.lambda().eq(true,SysAppPermission::getUrl,url).ne(oConvertUtils.isNotEmpty(id),SysAppPermission::getId,id).eq(true,SysAppPermission::isAlwaysShow,alwaysShow);
		return count(qw)==0;
	}
