...
import org.jeecg.modules.mobile.model.SysAppPermissionTree;
import org.jeecg.modules.mobile.model.TreeAppModel;
import org.jeecg.modules.mobile.service.ISysAppPermissionService;
...
@Slf4j
@RestController
@RequestMapping("/sys/appPermission")
public class SysAppPermissionController {

	@Autowired
	private ISysAppPermissionService sysPermissionService;
	...
	public Result<List<SysAppPermissionTree>> list(SysAppPermission sysPermission, HttpServletRequest req) {
		...
		try {
			...
			if(oConvertUtils.isNotEmpty(sysPermission.getName())){
				query.like(SysAppPermission::getName, sysPermission.getName());
			}
			List<SysAppPermission> list = sysPermissionService.list(query);
			...
			if(oConvertUtils.isNotEmpty(sysPermission.getName())){
				if(list!=null && list.size()>0){
					treeList = list.stream().map(e -> {
						e.setLeaf(true);
						return new SysAppPermissionTree(e);
	...
	public Result<List<SysAppPermissionTree>> getSystemMenuList() {
        long start = System.currentTimeMillis();
		Result<List<SysAppPermissionTree>> result = new Result<>();
		try {
			LambdaQueryWrapper<SysAppPermission> query = new LambdaQueryWrapper<SysAppPermission>();
			query.eq(SysAppPermission::getMenuType,CommonConstant.MENU_TYPE_0);
			query.eq(SysAppPermission::getDelFlag, CommonConstant.DEL_FLAG_0);
			query.orderByAsc(SysAppPermission::getSortNo);
			List<SysAppPermission> list = sysPermissionService.list(query);
			List<SysAppPermissionTree> sysPermissionTreeList = new ArrayList<SysAppPermissionTree>();
			for(SysAppPermission sysPermission : list){
				SysAppPermissionTree sysPermissionTree = new SysAppPermissionTree(sysPermission);
	...
	public Result<List<SysAppPermissionTree>> getSystemSubmenu(@RequestParam("parentId") String parentId){
		Result<List<SysAppPermissionTree>> result = new Result<>();
		try{
			LambdaQueryWrapper<SysAppPermission> query = new LambdaQueryWrapper<SysAppPermission>();
	...
	public Result getSystemSubmenuBatch(@RequestParam("parentIds") String parentIds) {
		try {
			...
			query.in(SysAppPermission::getParentId, parentIdList);
			query.eq(SysAppPermission::getDelFlag, CommonConstant.DEL_FLAG_0);
			query.orderByAsc(SysAppPermission::getSortNo);
			List<SysAppPermission> list = sysPermissionService.list(query);
			Map<String, List<SysAppPermissionTree>> listMap = new HashMap(5);
			for (SysAppPermission item : list) {
				String pid = item.getParentId();
				if (parentIdList.contains(pid)) {
					List<SysAppPermissionTree> mapList = listMap.get(pid);
					if (mapList == null) {
						mapList = new ArrayList<>();
					}
					mapList.add(new SysAppPermissionTree(item));
...
//	}
	...
	public Result<?> getUserPermissionByToken(HttpServletRequest request) {
		...
		try {
			...
			List<SysAppPermission> metaList = sysPermissionService.queryByUser(loginUser.getId());
			//添加首页路由
...
//				LambdaQueryWrapper<SysAppPermission> indexQueryWrapper = new LambdaQueryWrapper<>();
//				indexQueryWrapper.eq(SysAppPermission::getUrl, defIndexCfg.getUrl());
//				SysAppPermission indexMenu = sysPermissionService.getOne(indexQueryWrapper);
//				if (indexMenu == null) {
//					indexMenu = new SysAppPermission();
...
/* TODO 注： 这段代码的主要作用是：把首页菜单的组件替换成角色菜单的组件，由于现在的逻辑如果角色菜单不存在则自动插入一条，所以这段代码暂时不需要
			List<SysAppPermission> menus = metaList.stream().filter(sysPermission -> {
...
*/
			...
			//查询所有的权限
			LambdaQueryWrapper<SysAppPermission> query = new LambdaQueryWrapper<SysAppPermission>().select( SysAppPermission::getName, SysAppPermission::getPermsType, SysAppPermission::getPerms, SysAppPermission::getStatus);
			query.eq(SysAppPermission::getDelFlag, CommonConstant.DEL_FLAG_0);
			query.eq(SysAppPermission::getMenuType, CommonConstant.MENU_TYPE_2);
			//query.eq(SysAppPermission::getStatus, "1");
			List<SysAppPermission> allAuthList = sysPermissionService.list(query);
	...
	public Result<SysAppPermission> add(@RequestBody SysAppPermission permission) {
		...
		try {
			permission = intelligentProcessData(permission);
	...
	public Result<SysAppPermission> delete(@RequestParam(name = "id", required = true) String id) {
		Result<SysAppPermission> result = new Result<>();
		try {
	...
	public Result<SysAppPermission> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
		Result<SysAppPermission> result = new Result<>();
		try {
	...
	public Result<Map<String, Object>> queryTreeList() {
		...
		try {
			...
			List<TreeAppModel> treeList = new ArrayList<>();
	...
	public Result<List<TreeAppModel>> queryAsync(@RequestParam(name = "pid", required = false) String parentId) {
		Result<List<TreeAppModel>> result = new Result<>();
		try {
			List<TreeAppModel> list = sysPermissionService.queryListByParentId(parentId);
			if (list == null || list.size() <= 0) {
	...
	private void getTreeModelList(List<TreeAppModel> treeList, List<SysAppPermission> metaList, TreeAppModel temp) {
		for (SysAppPermission permission : metaList) {
			String tempPid = permission.getParentId();
			TreeAppModel tree = new TreeAppModel(permission);
			if (temp == null && oConvertUtils.isEmpty(tempPid)) {
				treeList.add(tree);
				if (!tree.getIsLeaf()) {
	...
	public static SysAppPermission intelligentProcessData(SysAppPermission permission) {
		if (permission == null) {
			return null;
		}
