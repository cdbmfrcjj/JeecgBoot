 ...
 */
@RestController
@RequestMapping("/sys/appRole")
@Slf4j
public class SysAppRoleController {
	@Autowired
	...
	@RequestMapping(value = "/queryAppTreeList", method = RequestMethod.GET)
	public Result<Map<String,Object>> queryAppTreeList(HttpServletRequest request) {
		Result<Map<String,Object>> result = new Result<>();
		//全部权限ids
		List<String> ids = new ArrayList<>();
		try {
			...
			List<SysAppPermission> list = sysAppPermissionService.list();
			for(SysAppPermission sysPer : list) {
				ids.add(sysPer.getId());
			}
			List<TreeAppModel> treeList = new ArrayList<>();
			getTreeModelAppList(treeList, list, null);
	...
	private void getTreeModelAppList(List<TreeAppModel> treeList, List<SysAppPermission> metaList, TreeAppModel temp) {
		for (SysAppPermission permission : metaList) {
			String tempPid = permission.getParentId();
			TreeAppModel tree = new TreeAppModel(permission.getId(), tempPid, permission.getName(),permission.getRuleFlag(), permission.isLeaf());
			if(temp==null && oConvertUtils.isEmpty(tempPid)) {
				treeList.add(tree);
				if(!tree.getIsLeaf()) {
					getTreeModelAppList(treeList, metaList, tree);
				}
			}else if(temp!=null && tempPid!=null && tempPid.equals(temp.getKey())){
				temp.getChildren().add(tree);
				if(!tree.getIsLeaf()) {
